...
\useunder{\uline}{\ul}{}
\graphicspath{{\Sexpr{paste0(loc_scripts,"/")}}} %so it can find NS logo
...
% colors are set up in the R script as the number of colors depends 
% on the number of algorithms. Give a place for the knitr hook to grab and replace
%CustomColorDefsHere
...
\begin{minipage}[b]{4.75in}   %everything in this minipage will be adjacent, left of the thermometer
  ...
  Grank: \Sexpr{ paste(as.character(ElementNames[[7]])," - ",as.character(grank_desc[[2]]), sep="")}\\
...
@
\end{center}
Table 2. Input statistics. Presence locations (= known locations) may be based on point observations (buffered based on their spatial accuracy) and polygon representations. Nearby locations are grouped together. Environmental conditions are randomly sampled within each of these groups the number of times noted (subsamples) to generate the total number of presence inputs (by tree for rf; for entire model for all others). Background points are placed throughout model area excluding presence locations. Algorithms incorporate background points differently, as noted. 
\begin{center}
...
<<tableThree, results="asis", echo = FALSE>>=
  print(xtable(vstats.w), 
        floating=FALSE, include.rownames=FALSE)
@
\end{center}

\end{minipage}%
\hfill \begin{minipage}[t]{3.3in}
\smallskip
<<importanceFig, fig.width=3.3, fig.height=6.3, include=FALSE, echo=FALSE>>=
...
\end{center}
Figure 1. Relative importance of environmental variables based on the full model. Importance values are extracted from each algorithm and plotted in descending order based on an average of all algorithms (grey line). Note each algorithm has variable removal rules (see Appendix 2); a missing point indicates the variable was not used by that algorithm. See Appendix 1 for variable descriptions.
\end{minipage}%
\normalsize
\pagebreak

\begin{center}
<<ROCplot, fig.width=4.5, fig.height=4.5, include=FALSE, echo=FALSE>>=
...
for(algo in ensemble_algos){
  ...
  else if(algo == "me" & exists("me.perf")){
    specs <- figSpecs[match(algo,figSpecs$algos),]
    plot(me.perf, 
         lwd=specs$lwd, 
         col=specs$col,
...
\end{center}
Figure 2. ROC plot for all \Sexpr{length(group$vals)} validation runs, 
...
\pagebreak
\noindent\textbf{Model Evaluation and Intended Use} \\
All SDMs are sensitive to data inputs and methodological choices. Table 5 presents scoring of modeling factors based on the model evaluation rubric presented in Sofaer et al. 2019 {\protect\NoHyper\cite{SofaerEtAl2019}\protect\endNoHyper}. \Sexpr{as.character(project_blurb)}\\
...
\\ \hline
  & Iterative 
  & \Sexpr{as.character(sdm.modeluse$iterative)} 
  & \Sexpr{as.character(sdm.modeluse$iterativeNotes)} 
\\ \hline                      
\end{tabular}
\end{table}
...
\includegraphics[width=1.0\linewidth]{nynhp-logo} %png logo file at repository root
\end{minipage}%
\begin{minipage}[c]{0.75\linewidth}
Please cite this document and its associated SDM as: \\
New York Natural Heritage Program. \Sexpr{format(Sys.Date(), "%Y")}. Species distribution model for \Sexpr{as.character(ElementNames$CommName)} (\textit{\Sexpr{as.character(ElementNames$SciName)}}). Created on \Sexpr{format(Sys.Date(), "%d %b %Y")}. Albany, NY.
\end{minipage}
}
