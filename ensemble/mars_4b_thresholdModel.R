...
totGPs.s <- length(unique(mars.df.full.s$group_id)[!grepl("pseu-a",unique(mars.df.full.s$group_id))])
...
totGPs.f <- length(unique(mars.df.full$group_id)[!grepl("pseu-a",unique(mars.df.full$group_id))])
