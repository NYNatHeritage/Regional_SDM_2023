...
#totGPs <- length(unique(rf.df.full[,group$colNm])[!grepl("pseu-a",unique(rf.df.full[,group$colNm]))]) <- This alternates what the "Group" is based on a min number of EOs in Script 3. So if there are fewer than 20 it jackknifes by poly instead, and then the group name is the stratum. But that means we never get a metric for how many EOs are included in the cutoff and I think that is less useful.
totGPs <- length(unique(rf.df.full$group_id)[!grepl("pseu-a",unique(rf.df.full$group_id))])
...
names(maxInEachGP) <- c("group_id","X1")
...
# get min of max values by Stratum (MTPP; minimum training by Poly ("stratum") presence)----
...
capturedGPs <- length(unique(allVotesPresPts$group_id))
capturedPolys <- length(unique(allVotesPresPts[allVotesPresPts$X1 >= MTPP,"stratum"]))
